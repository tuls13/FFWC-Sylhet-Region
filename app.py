...
def index():
    ...
    selected_station = request.args.get("station", "Jamalpur")
    if selected_station not in all_data:
        selected_station = next(iter(all_data), "Jamalpur")

    station_info = all_data.get(selected_station)
    data = station_info["data"] if station_info else []
    danger_level = station_info["danger_level"] if station_info else None
    ...
    if data:
        ...
        plt.title(f"Water Level Over Time - {selected_station}")
        plt.xlabel("Date")
